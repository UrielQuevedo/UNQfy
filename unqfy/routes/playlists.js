...
router.post('/', (connection.executeFunction(['name'],(unqfy, req, res) => {
  ...
  if(genres !== undefined && maxDuration !== undefined ) {
    ...
    unqfy.notifyAllObservers({ message: 'Se agrego la playlist ' + playlist.name, levelMessage: 'info'});
...
router.delete('/:id', (connection.executeFunction([],(unqfy, req, res) => {
  const { id } = req.params;
  unqfy.notifyAllObservers({ message: 'Se elimino la playlist con id ' + id, levelMessage: 'info'});
